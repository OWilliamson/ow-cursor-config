---
name: owcc-repo-convention-change
description: >-
...
Explicit-only. Invoke with `/owcc-repo-convention-change`.
...
**Defer instead of duplicating:** for CLI flags, packagers, and generated artifacts, apply the target repo’s **`AGENTS.md` → Tooling profile** after policy is clear. For auditing `AGENTS.md` / `.cursor/rules` / hubs holistically, use `/owcc-workflow-review`.

## Boundaries

- Policy and docs alignment only — not feature implementation unless required to enforce the convention.
- Holistic workflow audit → `/owcc-workflow-review`, not this skill.
- Repo cruft removal → `/owcc-repo-cleanup`, not this skill.
