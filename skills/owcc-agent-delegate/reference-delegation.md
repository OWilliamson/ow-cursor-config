...
| Deep PR review with rationale + alternatives | Main thread + `owcc-script-review` / team standards | Compressed one-liners only |
...
| Need | Skill / doc |
|------|-------------|
| Pick any hub tool | `/owcc-tooling-help` |
| Session friction retro | `/owcc-session-retro` |
| Ephemeral handoff note | `/chat-notation` (private) |
| Compress always-on context | `/owcc-context-compress` |

Versioned subagent notes: `subagents/README.md` in the publish tree (see `/owcc-agent-delegate` skill package for hub path).
