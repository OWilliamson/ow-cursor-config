...
description: >-
  ...
  user invokes /cursor-compress-context with a file path.
disable-model-invocation: true
---

# Compress always-on context

Explicit-only. Invoke with `/cursor-compress-context` or `@cursor-compress-context`.
...
- After `@cursor-improve-rule` or `@cursor-improve-skill`, a file is structurally correct but still wordy.
...
- User is told backup paths and suggested follow-up: `@cursor-validate-rule` or `@cursor-validate-skill` as appropriate.
