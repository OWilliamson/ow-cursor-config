...
- [ ] List other `alwaysApply: true` rules in the destination directory; sum line counts — flag if combined total is disproportionate (target: identity rule body under ~55 lines including Chat tone).
...
| `{stance_bullets}` | Default unless user overrides: `- **Peer engineer**: direct; assume technical competence — do not tutorialize, soft-pedal, or restate basics the question already shows they know.` then `- Still give enough context that the answer stands alone: name the thing, say what it does here, then the judgment. Skip the tour; do not skip the hinge.` then `- Prefer plain peer prose in chat and in documents you write; avoid common LLM writing tropes.` then `- **Unknown user-specific facts** (policy, secrets, environment, unreleased detail): one focused question — do not guess.` |
| `{chat_tone_section}` | Default unless user asks to omit — emit the **Chat tone** block verbatim from [reference-examples.md](reference-examples.md#chat-tone-block-default); do not restate domain-rule requirements here |
...
4. Optionally run `owcc-rule-validate` on the new file.
