...
Treat the injected line as a **signal**, not a full retro: open a **new chat**, run **`/owcc-session-retro`**, paste **redacted** excerpts from the JSONL row or the original session, then run the retro workflow in the main skill.
