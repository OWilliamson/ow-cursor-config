...
Explicit-only. Invoke with `/owcc-session-retro`.
...
- Do not implement remediation (rules, skills, code) unless the user starts a **separate** fix request.
- Do not ask for or log raw secrets, tokens, or private keys.
- Single-turn incident on the last exchange → `/agent-problem-log` (private profile), not this skill.
